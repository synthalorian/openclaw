...
export async function runCopilotIsolatedCompletion(
  ...
  if (params.streamParams?.temperature !== undefined) {
    throw new Error("[copilot] isolated completion does not support temperature");
  }
  ...
  if (params.streamParams?.maxTokens !== undefined && resolvedProvider.mode === "github-copilot") {
    throw new Error("[copilot] native isolated completion does not support maxTokens");
  }
  ...
  try {
    ...
    if (event.data.toolRequests?.length) {
      throw new Error("[copilot] isolated completion returned a tool request");
    }
